...
### 재귀함수
- 비트마스크와 마찬가지로 각 원소가 두 가지 선택지를 가질 때 유용하게 사용할 수 있다.
- 포함이 되면 해당 원소를 넣어 함수를 호출하고, 포함되지 않으면 그 상태에서 함수를 호출하는 방식이다. 종료 조건이 명시되어야 재귀함수에서 빠져나올 수 있다.
- 가장 간단한 형태의 피보나치 수열은 재귀함수의 형태로 풀릴 수 있다.
  ```python
  def fibo(x):
